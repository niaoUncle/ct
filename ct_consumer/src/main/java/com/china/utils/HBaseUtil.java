...
public class HBaseUtil {
    ...
    public static void createTable( Configuration conf ,String tableName,String ...columnFamily){
        ...
        try {
            ...
            if(isExistTable(conf, tableName)){
                ...
                int regions = Integer.valueOf(PropertiesUtil.getproperties("hbase.regions.count"));

                admin.createTable(tableDescriptor,getSplitKey(regions));

                System.out.print("表创建成功：" + tableName);
            }
    ...
    public static byte[][] getSplitKey(int regions){
        ...
        Iterator<byte[]> rowKeysIter = rows.iterator();

        int i =0;

        while(rowKeysIter.hasNext()){

                byte[] tempRow = rowKeysIter.next();
